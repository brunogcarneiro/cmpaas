...
var classXclassLinks           = ["are", "equivalent to", "cannot be", "exact opposite of", "is composed of","is an attribute of", "that is"];
...
CMPAAS.editor = function() {
  ...
  public.init = function() { 
    ...
    var individualNodeTemplate = $$(go.Node, "Auto",
        new go.Binding("layerName", "layer"),
        new go.Binding("location", "loc", go.Point.parse).makeTwoWay(go.Point.stringify),
        // define the node's outer shape, which will surround the TextBlock
        $$(go.Shape, "Rectangle",
  ...
  myDiagram.contextMenu = 
    ...
    myDiagram.model.addChangedListener(function(e){
      ...
      if (e.change == go.ChangedEvent.Property){
        console.log("alterou link: " +e.propertyName+":"+e.object.text + ":" + e.object.category);
      ...
      if (e.change == go.ChangedEvent.Insert
        ...
        var m = e.object;
        var l = m.linkDataArray[e.newParam];
        ...
        var fcat = f.category;
        var tcat = t.category;
        m.setDataProperty(l,"text","are");
        
        //m.setDataProperty(l,"category","predefined");
        ...
        else if ( fcat === "individualNode")
        {
          ...
          {
            m.setDataProperty(l,"text","same as");
            return;
          } 
          else (tcat === "classNode") 
          {
            m.setDataProperty(l,"text","is a");
            return;
          }
        } 
        else 
      ...
      if (e.change == go.ChangedEvent.Property 
        ...
        && e.object.from != null) {
          ...
          var new_category = getLinkCategoryByName(e.newValue);
          var old_category = getLinkCategoryByName(e.oldValue);
...
function checkLinksRule(link){
  ...
  var name = link.text;
  var index = -1;
  var category = getLinkCategoryByName(name);
...
function getLinkCategoryByName(name){
  var options = elemento.children.namedItem("lista").options;

  for (var i = 0 ; i < options.length ; i++){
    op = options[i];
    if (op.value == name) {
      return options[i].getAttribute("category");
    }
  }
  return "";
}
